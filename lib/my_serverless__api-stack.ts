...
export class MyServerlessApiStack extends cdk.Stack {
  constructor (scope: Construct, id: string, props?: cdk.StackProps) {
    ...
    const textReadAllHandler = new cdk.aws_lambda_nodejs.NodejsFunction(
      this,
      'text-read-all',
      {
        timeout: cdk.Duration.seconds(5),
        memorySize: 1024,
        entry: 'code/text-read-all/index.ts',
        environment: {
          BUCKET_NAME: myBucket.bucketName
        }
      }
    )
    myBucket.grantRead(textReadAllHandler)
    ...
    texts.addMethod(
      'GET',
      new cdk.aws_apigateway.LambdaIntegration(textReadAllHandler)
    )
